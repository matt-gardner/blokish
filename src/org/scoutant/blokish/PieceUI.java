...
public class PieceUI extends FrameLayout implements OnTouchListener, OnLongClickListener, Comparable<PieceUI> {
    ...
    public void move(int i, int j) {
        this.i=i;
        this.j=j;
        // Caution : Must invoque doLayout every time i and j is modified! invalidate() and
        // onDraw() will operate only if "piece is in viewport". Which will be reevaluated with
        // doLayout()!
        doLayout();
        invalidate();
    }
    ...
    /**
     * Caution : must invoke doLayout() before any invalidate() if i or j happened to be updated!
     * As onDraw wont be called if piece is (was) out of  viewport.
     */
    @Override
